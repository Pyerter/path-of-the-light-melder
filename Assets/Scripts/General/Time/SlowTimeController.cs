...
namespace TimeSlowing
{
    public class TargetedSlowTime <T> where T : SlowTime<T>
    {
        ...
        public void AddTargets(params IIdentifiable[] targets)
        {
            foreach (IIdentifiable target in targets)
                if (!this.targets.Contains(target.ID))
        ...
        {
            return targets.Contains(identifiable.ID);
        }
    ...
    {
        protected float controlledTime = 0f;
        public float ControlledTime { get { return controlledTime; } }
        ...
        {
            if (!targetedSlowSubscribers.ContainsKey(identifiable.ID))
                targetedSlowSubscribers.Add(identifiable.ID, new TargetedSlowInfo<T>());
        }

        public System.Action<float> SubscribeTargetedSlow(IIdentifiable identifiable, System.Action<float> onSlow)
        {
            if (targetedSlowSubscribers.TryGetValue(identifiable.ID, out TargetedSlowInfo<T> tsi))
            {
                System.Action<float> currentOnSlow = tsi.OnSlow;
                targetedSlowSubscribers[identifiable.ID].OnSlow = onSlow;
                return currentOnSlow;
            }
            else
            {
                targetedSlowSubscribers.Add(identifiable.ID, new TargetedSlowInfo<T>(onSlow));
                return null;
            }
        }
        ...
        {
            return targetedSlowSubscribers.Remove(identifiable.ID);
        }
        ...
        public virtual void Tick()
        {
            ...
            float deltaTime = UpdateType == SlowUpdateType.FIXED ? Time.fixedDeltaTime : Time.deltaTime;
            controlledTime += TimeScale * deltaTime;
        }
